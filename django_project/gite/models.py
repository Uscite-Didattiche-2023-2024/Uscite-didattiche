...
class Gita(models.Model):
    ...
    Stato = models.CharField(
        max_length=20, choices=STATO_CHOICES, default="IN_PROGRAMMA"
    )
...
class Proposta_Gita(models.Model):
    ...
    Stato = models.CharField(
        max_length=20, choices=STATO_CHOICES, default="IN_ELABORAZIONE"
    )
...
class Notifica(models.Model):
    ...
    Gita = models.ForeignKey("Gita", on_delete=models.CASCADE, default="")
    Proposta_Gita = models.ForeignKey(
        "Proposta_Gita", on_delete=models.CASCADE, default=""
    )
    Classe = models.ManyToManyField("Classe", related_name="notifica")
    Documenti = models.ForeignKey(
        "Documenti", on_delete=models.CASCADE, default="", blank=True, null=True
    )
    utenti_letto = models.ManyToManyField(
        User, related_name="notifiche_lette", blank=True
    )
