...
class Proposta_gitaUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def test_func(self):
        ...
        return self.request.user == proposta.Creatore or self.request.user.has_perm(
            "gite.change_proposta_gita"
        )
...
class GitaCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        for classe_id in classi_selezionate:
            classe_gita = Classe_gita.objects.create(
                Gita=self.object, Classe_id=classe_id
            )
            classe_gita.save()
        ...
        gruppi = Group.objects.filter(
            name__in=["Studente minorenne", "Studente maggiorenne", "Genitore"]
        )
...
class Rifiuta_proposta(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def get(self, request, pk):
        ...
        return render(
            request,
            "gite/proposta_gita_detail.html",
            {"proposta": proposta, "notifiche": notifiche},
        )
...
class GitaUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        classi_da_elimare = classi_gita_esistenti.exclude(
            Classe_id__in=classi_selezionate
        )
...
def aboutUs(request):
    notifiche = Notifica.objects.all()
    return render(
        request, "gite/aboutUs.html", {"title": "AboutUs", "notifiche": notifiche}
    )
...
class GiteListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if user.has_perm("gite.view_gita"):
            ...
            queryset = (
                Gita.objects.all()
                .select_related("Proposta_Gita__Creatore")
            ...
            if user_classe:
                queryset = (
                    Gita.objects.filter(classe_gita__Classe=user_classe)
                    .select_related("Proposta_Gita__Creatore")
                    .order_by("-Data_ritrovo")
                )
            else:
                queryset = (
                    Gita.objects.none()
                )  # Nessuna gita se l'utente non ha una classe assegnata
...
class CustomHTMLCalendar(HTMLCalendar):
    ...
    def formatday(self, day, weekday, year, month):
        if day == 0:
            ...
            gite_list = "".join(
                f'<div><a href="javascript:void(0);" onclick="loadEventDetails({gita.id})">{gita.Proposta_Gita.Titolo}</a></div>'
                for gita in gite
            )
...
class CalendarioView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        previous_month_link = (
            f"/calendario?year={previous_month.year}&month={previous_month.month}"
        )
